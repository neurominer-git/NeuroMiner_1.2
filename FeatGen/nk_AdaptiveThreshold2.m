...
[~, ~, ~, ~, func] = nk_ReturnEvalOperator(SVM.GridParam);
...
for curclass=1:nclass
    ...
    for k=lpercvec:-1:1
        ...
        switch adap.method
            ...
            case 0 % (CV1-Training Perf. + CV1-Test Perf.) / 2 Criterion
                ...
                trainval.acc = EVALFUNC(labels, YPred(:,k,curclass));
                valt = EVALFUNC(labelnew, YnewPred(:,k,curclass))
                fprintf('(CV1-Tr+CV1-Ts)/2: percentile = %1.3f%%, threshold=%1.3f, #features = %g, Train-Perf = %1.2f, CV1-Perf = %1.2f (%s)', ...
                    percvec(k), thresh, sumfeat(k,curclass), trainval.acc, valt(1), char(EVALFUNC));
            ...
            case 1 % LOO validation Criterion
                ...
                trainval.acc = EVALFUNC(labels, pred); valt = trainval.acc;
                fprintf('LOO: percentile = %1.3f%%, threshold=%1.3f, #features = %g, LOO-Perf = %1.2f (%s)', ...
                    percvec(k), thresh, sumfeat(k,curclass), trainval.acc, char(EVALFUNC));
            ...
            case 2 % CV1-Test Perf. Criterion
                ...
                fprintf('CV1-Ts: percentile = %1.3f%%, threshold=%1.3f, #features = %g, CV1-Perf = %1.2f (%s)', ...
                    percvec(k), thresh, sumfeat(k,curclass), valt(1), char(EVALFUNC));
        end
    ...
    if size(V,2) > 1,
        [mx1,in1] = func(trainperf(:,curclass));
        [mx2,in2] = func(perf(:,curclass));
