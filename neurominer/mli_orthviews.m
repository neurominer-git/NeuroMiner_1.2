...
%                    - currently always 1
% 
%         stMLI.registry.hReg \_ See spm_XYZreg for documentation
%         stMLI.registry.hMe  /
% 
...
%                menu.
% b) at redraw:  After images and blobs of stMLI.vols{i} are drawn, the
%                struct stMLI.vols{i} is checked for field names that occur in
%                the plugin list stMLI.plugins{:}. For each matching entry, the
...
if ~isfield(stMLI,'vols'),reset_st; end
if isempty(stMLI) reset_st; end
spm('Pointer','watch');
...
case 'image',
	...
	if ~isempty(H)
		stMLI.vols{H}.area = [0 0 1 1];
		if length(varargin)>=2, stMLI.vols{H}.area = varargin{2}; end;
		if isempty(stMLI.bb), stMLI.bb = maxbb; end;
		bbox; 
		cm_pos;
	end;
	varargout{1} = H;
	stMLI.centre    = mean(maxbb);
	redraw_all

case 'bb',
	if length(varargin)> 0 && all(size(varargin{1})==[2 3]), stMLI.bb = varargin{1}; end;
	bbox;
	redraw_all;
...
case 'redraw',
	redraw_all;
	eval(stMLI.callback);
	if isfield(stMLI,'registry'),
		spm_XYZreg('SetCoords',stMLI.centre,stMLI.registry.hReg,stMLI.registry.hMe);
	end;
...
case 'reposition',
	...
	if length(tmp)==3,
                h = valid_handles(stMLI.snap);
                if ~isempty(h)
                        tmp=stMLI.vols{h(1)}.mat*...
                            round(inv(stMLI.vols{h(1)}.mat)*[tmp; ...
                                            1]);
                end;
                stMLI.centre = tmp(1:3); 
	end;
	redraw_all;
	eval(stMLI.callback);
	if isfield(stMLI,'registry'),
...
case 'setcoords',
	stMLI.centre = varargin{1};
	stMLI.centre = stMLI.centre(:);
	redraw_all;
	eval(stMLI.callback);
	cm_pos;
...
otherwise,
  addonaction = strcmp(stMLI.plugins,action);
  if any(addonaction)
    feval(['spm_ov_' stMLI.plugins{addonaction}],varargin{:});
  end;
end;
...
for i=valid_handles(handle),
	if ~isempty(xyz),
		...
		vol(off) = t;
		vol      = reshape(vol,dim);
		stMLI.vols{i}.blobs=cell(1,1);
		mx = max([eps max(t)]);
		mn = min([0 min(t)]);
		stMLI.vols{i}.blobs{1} = struct('vol',vol,'mat',mat,'max',mx, 'min',mn,'name',name);
...
for i=valid_handles(handle),
	...
	mn = min([0 minval(vol)]);
	stMLI.vols{i}.blobs{1} = struct('vol',vol,'mat',mat,'max',mx,'min',mn);
	addcolourbar(handle,1);
end;
return;
%_______________________________________________________________________
...
for i=valid_handles(handle),
	if ~isempty(xyz),
		...
		vol(off) = t;
		vol      = reshape(vol,dim);
		if ~isfield(stMLI.vols{i},'blobs'),
			stMLI.vols{i}.blobs=cell(1,1);
			bset = 1;
		else
			bset = length(stMLI.vols{i}.blobs)+1;
		end;
                mx = max([eps maxval(vol)]);
                mn = min([0 minval(vol)]);
		stMLI.vols{i}.blobs{bset} = struct('vol',vol, 'mat',mat, ...
                                                'max',mx, 'min',mn, ...
...
for i=valid_handles(handle),
	...
	mx = max([eps maxval(vol)]);
	mn = min([0 minval(vol)]);
	stMLI.vols{i}.blobs{bset} = struct('vol',vol,'mat',mat,'max',mx,'min',mn,'colour',colour);
end;
return;
%_______________________________________________________________________
%_______________________________________________________________________
...
for i=valid_handles(handle),
	...
	stMLI.vols{i}.blobs{bset} = struct('vol',vol,'mat',mat,'max',mx, ...
                                        'min',mn,'colour',c);
...
global stMLI
if stMLI.mode == 0,
    axpos = get(stMLI.vols{vh}.ax{2}.ax,'Position');
else
    axpos = get(stMLI.vols{vh}.ax{1}.ax,'Position');
end;
stMLI.vols{vh}.blobs{bh}.cbar = axes('Parent',stMLI.fig,...
          'Position',[(axpos(1)+axpos(3)+0.05+(bh-1)*.1) (axpos(2)+0.005) 0.05 (axpos(4)-0.01)],...
          'Box','on', 'YDir','normal', 'XTickLabel',[], 'XTick',[]);
if isfield(stMLI.vols{vh}.blobs{bh},'name')
    ylabel(stMLI.vols{vh}.blobs{bh}.name,'parent',stMLI.vols{vh}.blobs{bh}.cbar);
end;
return;
%_______________________________________________________________________
%_______________________________________________________________________
...
for i=valid_handles(handle),
	if isfield(stMLI.vols{i},'blobs'),
		for j=1:length(stMLI.vols{i}.blobs),
			if isfield(stMLI.vols{i}.blobs{j},'cbar') && ishandle(stMLI.vols{i}.blobs{j}.cbar),
				delete(stMLI.vols{i}.blobs{j}.cbar);
			end;
		end;
		stMLI.vols{i} = rmfield(stMLI.vols{i},'blobs');
	end;
end;
return;
%_______________________________________________________________________
...
global stMLI
tmp = uicontrol('Position',[0 0 1 1],'Visible','off','Parent',stMLI.fig);
...
stMLI.centre = spm_XYZreg('GetCoords',stMLI.registry.hReg);
stMLI.centre = stMLI.centre(:);
return;
%_______________________________________________________________________
%_______________________________________________________________________
...
for arg1=valid_handles(arg1),
	is = inv(stMLI.vols{arg1}.premul*stMLI.vols{arg1}.mat);
	H = is(1:3,1:3)*stMLI.centre(:) + is(1:3,4);
end;
return;
%_______________________________________________________________________
%_______________________________________________________________________
function my_reset
global stMLI
if ~isempty(stMLI) && isfield(stMLI,'registry') && ishandle(stMLI.registry.hMe),
	delete(stMLI.registry.hMe); stMLI = rmfield(stMLI,'registry');
end;
my_delete(1:24);
reset_st;
return;
...
for i=valid_handles(arg1),
	kids = get(stMLI.fig,'Children');
	for j=1:3,
		if any(kids == stMLI.vols{i}.ax{j}.ax),
			set(get(stMLI.vols{i}.ax{j}.ax,'Children'),'DeleteFcn','');
			delete(stMLI.vols{i}.ax{j}.ax);
		end;
	end;
	stMLI.vols{i} = [];
end;
...
%_______________________________________________________________________
%_______________________________________________________________________
function my_cla(arg1)
global stMLI
for i=valid_handles(arg1),
	for j=1:3,
        %set(get(stMLI.vols{i}.ax{j}.ax,'Children'),'DeleteFcn','');
        cla(stMLI.vols{i}.ax{j}.ax,'reset');
	end;
	stMLI.vols{i} = [];
end;
return;
...
global stMLI
res      = arg1/mean(svd(stMLI.Space(1:3,1:3)));
Mat      = diag([res res res 1]);
stMLI.Space = stMLI.Space*Mat;
stMLI.bb    = stMLI.bb/res;
return;
%_______________________________________________________________________
%_______________________________________________________________________
...
for i=valid_handles(1:24),
	...
	tc = stMLI.Space\(stMLI.vols{i}.premul*stMLI.vols{i}.mat)*c;
	tc = tc(1:3,:)';
	mx = max([tc ; mx]);
	mn = min([tc ; mn]);
end;
...
global stMLI
if ~isempty(stMLI.vols{arg1})
	num = arg1;
        if nargin < 2
	    M = stMLI.vols{num}.mat;
	    dim = stMLI.vols{num}.dim(1:3);
	end;
	Mat = stMLI.vols{num}.premul(1:3,1:3)*M(1:3,1:3);
	vox = sqrt(sum(Mat.^2));
...
function bbox
global stMLI
Dims = diff(stMLI.bb)'+1;

TD = Dims([1 2])';
CD = Dims([1 3])';
if stMLI.mode == 0, SD = Dims([3 2])'; else SD = Dims([2 3])'; end;
un    = get(stMLI.fig,'Units');set(stMLI.fig,'Units','Pixels');
sz    = get(stMLI.fig,'Position');set(stMLI.fig,'Units',un);
sz    = sz(3:4);
sz(2) = sz(2)-40;

for i=valid_handles(1:24),
	area = stMLI.vols{i}.area(:);
	...
	% Coronal
	set(stMLI.vols{i}.ax{2}.ax,'Units','Pixels',...
	...
	% Sagittal
	if stMLI.mode == 0,
		set(stMLI.vols{i}.ax{3}.ax,'Units','Pixels', 'Box','on',...
...
is   = inv(stMLI.Space);
cent = is(1:3,1:3)*stMLI.centre(:) + is(1:3,4);

for i = valid_handles(arg1),
	M = stMLI.vols{i}.premul*stMLI.vols{i}.mat;
	TM0 = [	1 0 0 -bb(1,1)+1
		0 1 0 -bb(1,2)+1
		0 0 1 -cent(3)
		0 0 0 1];
	TM = inv(TM0*(stMLI.Space\M));
	TD = Dims([1 2]);
	...
	if stMLI.mode ==0,
		SM0 = [	0 0 1 -bb(1,3)+1
			...
			1 0 0 -cent(1)
			0 0 0 1];
		SM = inv(SM0*(stMLI.Space\M)); SD = Dims([3 2]);
	else
		SM0 = [	0  1 0 -bb(1,2)+1
			0  0 1 -bb(1,3)+1
			1  0 0 -cent(1)
		...
		SM0 = [	0 -1 0 +bb(2,2)+1
			...
			1  0 0 -cent(1)
			0  0 0 1];
		SM = inv(SM0*(stMLI.Space\M));
		SD = Dims([2 3]);
	end;

	try
		imgt  = spm_slice_vol(stMLI.vols{i},TM,TD,stMLI.hld)';
		imgc  = spm_slice_vol(stMLI.vols{i},CM,CD,stMLI.hld)';
		imgs  = spm_slice_vol(stMLI.vols{i},SM,SD,stMLI.hld)';
		ok    = true;
	catch
	...
	if ok,
                ...
                if strcmp(stMLI.vols{i}.window,'auto')
                        mn = -Inf;
                        mx = Inf;
                else
                        mn = min(stMLI.vols{i}.window);
                        mx = max(stMLI.vols{i}.window);
                end;
                % threshold images
                ...
                if license('test','image_toolbox') == 0
                    stMLI.vols{i}.mapping = 'linear';
                end;
                switch stMLI.vols{i}.mapping,
                 case 'linear',
                 case 'histeq',
		...
		if isfield(stMLI.vols{i},'blobs'),
			if ~isfield(stMLI.vols{i}.blobs{1},'colour'),
				...
				if isfield(stMLI.vols{i}.blobs{1},'max'),
					mx = stMLI.vols{i}.blobs{1}.max;
				else
					mx = max([eps maxval(stMLI.vols{i}.blobs{1}.vol)]);
					stMLI.vols{i}.blobs{1}.max = mx;
				end;
				if isfield(stMLI.vols{i}.blobs{1},'min'),
					mn = stMLI.vols{i}.blobs{1}.min;
				else
					mn = min([0 minval(stMLI.vols{i}.blobs{1}.vol)]);
					stMLI.vols{i}.blobs{1}.min = mn;
				end;

				vol  = stMLI.vols{i}.blobs{1}.vol;
				M    = stMLI.vols{i}.premul*stMLI.vols{i}.blobs{1}.mat;
				tmpt = spm_slice_vol(vol,inv(TM0*(stMLI.Space\M)),TD,[0 NaN])';
				tmpc = spm_slice_vol(vol,inv(CM0*(stMLI.Space\M)),CD,[0 NaN])';
				tmps = spm_slice_vol(vol,inv(SM0*(stMLI.Space\M)),SD,[0 NaN])';
				...
				cmap = get(stMLI.fig,'Colormap');
				if size(cmap,1)~=128
					figure(stMLI.fig)
					spm_figure('Colormap','gray-hot')
				end;
                                redraw_colourbar(i,1,[mn mx],(1:64)'+64); 
			elseif isstruct(stMLI.vols{i}.blobs{1}.colour),
				...
				actc = ...
				    stMLI.vols{1}.blobs{1}.colour.cmap;
				actp = ...
				    stMLI.vols{1}.blobs{1}.colour.prop;
				...
				vol = stMLI.vols{i}.blobs{1}.vol;
				mat = stMLI.vols{i}.premul*stMLI.vols{i}.blobs{1}.mat;
				if isfield(stMLI.vols{i}.blobs{1},'max'),
					cmx = stMLI.vols{i}.blobs{1}.max;
				else
					cmx = max([eps maxval(stMLI.vols{i}.blobs{1}.vol)]);
				end;
				if isfield(stMLI.vols{i}.blobs{1},'min'),
					cmn = stMLI.vols{i}.blobs{1}.min;
				else
					cmn = -cmx;
				end;
				...
				for j=1:length(stMLI.vols{i}.blobs), % get colours of all images first
					if isfield(stMLI.vols{i}.blobs{j},'colour'),
						colour(j,:) = reshape(stMLI.vols{i}.blobs{j}.colour, [1 3]);
					else
						colour(j,:) = [1 0 0];
					end;
				end;
				...
				for j=1:length(stMLI.vols{i}.blobs),
					if isfield(stMLI.vols{i}.blobs{j},'max'),
						mx = stMLI.vols{i}.blobs{j}.max;
					else
						mx = max([eps max(stMLI.vols{i}.blobs{j}.vol(:))]);
						stMLI.vols{i}.blobs{j}.max = mx;
					end;
					if isfield(stMLI.vols{i}.blobs{j},'min'),
						mn = stMLI.vols{i}.blobs{j}.min;
					else
						mn = min([0 min(stMLI.vols{i}.blobs{j}.vol(:))]);
						stMLI.vols{i}.blobs{j}.min = mn;
					end;

					vol  = stMLI.vols{i}.blobs{j}.vol;
					M    = stMLI.Space\stMLI.vols{i}.premul*stMLI.vols{i}.blobs{j}.mat;
                                        ...
                                        % against mn/mx as given in stMLI
                                        tmpt(tmpt(:)<mn) = mn;
                                        tmpc(tmpc(:)<mn) = mn;
                                        tmps(tmps(:)<mn) = mn;
                                        tmpt(tmpt(:)>mx) = mx;
		...
		set(stMLI.vols{i}.ax{1}.d,'HitTest','off', 'Cdata',imgt);
		set(stMLI.vols{i}.ax{1}.lx,'HitTest','off',...
...
% 			'Ydata',[0 TD(2)]+0.5,'Xdata',[1 1]*(cent(1)-bb(1,1)+1));
		...
		set(stMLI.vols{i}.ax{2}.ly,'HitTest','off',...
			'Ydata',[0 CD(2)]+0.5,'Xdata',[1 1]*(cent(1)-bb(1,1)+1));

		set(stMLI.vols{i}.ax{3}.d,'HitTest','off','Cdata',imgs);
		if stMLI.mode ==0,
			set(stMLI.vols{i}.ax{3}.lx,'HitTest','off',...
				'Xdata',[0 SD(1)]+0.5,'Ydata',[1 1]*(cent(2)-bb(1,2)+1));
			set(stMLI.vols{i}.ax{3}.ly,'HitTest','off',...
		...
		if ~isempty(stMLI.plugins) % process any addons
			for k = 1:numel(stMLI.plugins),
				if isfield(stMLI.vols{i},stMLI.plugins{k}),
					feval(['spm_ov_', stMLI.plugins{k}], ...
						'redraw', i, TM0, TD, CM0, CD, SM0, SD);
				end;
			end;
...
global stMLI
if isfield(stMLI.vols{vh}.blobs{bh},'cbar')
    if stMLI.mode == 0,
        axpos = get(stMLI.vols{vh}.ax{2}.ax,'Position');
    else
        axpos = get(stMLI.vols{vh}.ax{1}.ax,'Position');
    ...
    image([0 1],interval,cdata,'Parent',stMLI.vols{vh}.blobs{bh}.cbar);
    set(stMLI.vols{vh}.blobs{bh}.cbar, ...
...
global stMLI
if isfield(stMLI,'ParentFig')
    obj    = get(stMLI.ParentFig,'CurrentObject');
else
    obj    = get(stMLI.fig,'CurrentObject');
end
centre = [];
cent   = [];
cp     = [];
for i=valid_handles(1:24),
	for j=1:3,
		...
		if ~isempty(cp),
			...
			switch j,
				case 1,
				cent([1 2])=[cp(1)+stMLI.bb(1,1)-1 cp(2)+stMLI.bb(1,2)-1];
				case 2,
				cent([1 3])=[cp(1)+stMLI.bb(1,1)-1 cp(2)+stMLI.bb(1,3)-1];
				case 3,
				if stMLI.mode ==0,
					cent([3 2])=[cp(1)+stMLI.bb(1,3)-1 cp(2)+stMLI.bb(1,2)-1];
				else
					cent([2 3])=[stMLI.bb(2,2)+1-cp(1) cp(2)+stMLI.bb(1,3)-1];
				end;
			end;
			break;
		end;
...
if ~isempty(cent), centre = stMLI.Space(1:3,1:3)*cent(:) + stMLI.Space(1:3,4); end;
return;
%_______________________________________________________________________
%_______________________________________________________________________
...
for h=handles,
	if isempty(stMLI.vols{h}), handles(handles==h)=[]; end;
end;
return;
%_______________________________________________________________________
%_______________________________________________________________________
...
if isfield(stMLI,'NMaxes')
    stMLI      = struct('n', 0, 'vols',[], 'bb',bb,'Space',eye(4),'centre',[0 0 0],'callback',';','xhairs',1,'hld',1,'fig',stMLI.fig,'mode',1,'plugins',[],'snap',[], 'NMaxes', stMLI.NMaxes, 'ParentFig', stMLI.ParentFig);
else
    if ~isfield(stMLI,'fig'), stMLI.fig = spm_figure('Findwin','Graphics'); end
    stMLI      = struct('n', 0, 'vols',[], 'bb',bb,'Space',eye(4),'centre',[0 0 0],'callback',';','xhairs',1,'hld',1,'fig',stMLI.fig,'mode',1,'plugins',[],'snap',[]);
end
...
for i=1:3,
    set(stMLI.vols{volhandle}.ax{i}.ax,'UIcontextmenu',item_parent);
    stMLI.vols{volhandle}.ax{i}.cm = item_parent;
end;

if ~isempty(stMLI.plugins) % process any plugins
	for k = 1:numel(stMLI.plugins),
		feval(['spm_ov_', stMLI.plugins{k}], ...
...
case 'image_info',
	...
	if isfield(stMLI.vols{current_handle},'fname'),
		[p,n,e,v] = spm_fileparts(stMLI.vols{current_handle}.fname);
                if isfield(stMLI.vols{current_handle},'n')
                    v = sprintf(',%d',stMLI.vols{current_handle}.n);
                end;
	...
	if isfield(stMLI.vols{current_handle},'descrip'),
		item2 = uimenu(varargin{2}, 'Label',...
		stMLI.vols{current_handle}.descrip);
	end;
	dt = stMLI.vols{current_handle}.dt(1);
	item3 = uimenu(varargin{2}, 'Label', sprintf('Data type: %s', spm_type(dt)));
	str   = 'Intensity: varied';
	if size(stMLI.vols{current_handle}.pinfo,2) == 1,
		if stMLI.vols{current_handle}.pinfo(2),
			str = sprintf('Intensity: Y = %g X + %g',...
				stMLI.vols{current_handle}.pinfo(1:2)');
		else
			str = sprintf('Intensity: Y = %g X', stMLI.vols{current_handle}.pinfo(1)');
		end;
	end;
	item4  = uimenu(varargin{2}, 'Label',str);
	item5  = uimenu(varargin{2}, 'Label', 'Image dims', 'Separator','on');
	item51 = uimenu(varargin{2}, 'Label',...
		sprintf('%dx%dx%d', stMLI.vols{current_handle}.dim(1:3)));
	prms   = spm_imatrix(stMLI.vols{current_handle}.mat);
...
case 'snap',
	...
	elseif varargin{2} == 2,
		...
		stMLI.snap = get_current_handle;
		z_handle = get(findobj(stMLI.vols{get_current_handle}.ax{1}.cm,'label','Snap to Grid'),'Children');
...
case 'window_gl',
	if varargin{2} == 2,
		for i = 1:length(get_cm_handles),
			stMLI.vols{i}.window = 'auto';
		end;
	else
		current_handle = get_current_handle;
		if isnumeric(stMLI.vols{current_handle}.window)
			defstr = sprintf('%d %d', stMLI.vols{current_handle}.window);
		else
			defstr = '';
		end;
...
case 'mapping',
	...
	current_handle = get_current_handle;        
        cm_handles = get_cm_handles;
        stMLI.vols{current_handle}.mapping = varargin{2};
...
case 'mapping_gl',
        ...
        for k = valid_handles(1:24),
                stMLI.vols{k}.mapping = varargin{2};
...
case 'add_c_blobs',
	...
	for i = 1:length(cm_handles),
		addcolouredblobs(cm_handles(i),VOL.XYZ,VOL.Z,VOL.M,colours(c,:),VOL.title);
                addcolourbar(cm_handles(i),numel(stMLI.vols{cm_handles(i)}.blobs));
		c_handle    = findobj(findobj(stMLI.vols{cm_handles(i)}.ax{1}.cm,'label','Blobs'),'Label','Remove colored blobs');
...
case 'remove_c_blobs',
    ...
    c_names = {'red';'yellow';'green';'cyan';'blue';'magenta'};
    for i = 1:length(cm_handles),
        if isfield(stMLI.vols{cm_handles(i)},'blobs'),
            for j = 1:length(stMLI.vols{cm_handles(i)}.blobs),
                if all(stMLI.vols{cm_handles(i)}.blobs{j}.colour == colours(varargin{3},:));
                    if isfield(stMLI.vols{cm_handles(i)}.blobs{j},'cbar')
                        delete(stMLI.vols{cm_handles(i)}.blobs{j}.cbar);
                    end
                    stMLI.vols{cm_handles(i)}.blobs(j) = [];
                    break;
                end;
            end;
            rm_c_menu = findobj(stMLI.vols{cm_handles(i)}.ax{1}.cm,'Label','Remove colored blobs');
            delete(gcbo);
            if isempty(stMLI.vols{cm_handles(i)}.blobs),
                stMLI.vols{cm_handles(i)} = rmfield(stMLI.vols{cm_handles(i)},'blobs');
...
for i = 1:length(valid_handles(1:24)),
	if isfield(stMLI.vols{i}.ax{1},'cm')
		...
		set(findobj(stMLI.vols{i}.ax{1}.cm,'UserData','v_value'),...
			'Label',sprintf('Y = %g',spm_sample_vol(stMLI.vols{i},pos(1),pos(2),pos(3),stMLI.hld)));
	end
end;
return;
%_______________________________________________________________________
...
cm_handles = [];
for i=valid_handles(1:24),
	cm_handles = [cm_handles stMLI.vols{i}.ax{1}.cm];
end
return;
%_______________________________________________________________________
%_______________________________________________________________________
...
if op==6,
	...
	elseif op == 4, stMLI.bb = [pos-40*vx ; pos+40*vx] ;
	elseif op == 3, stMLI.bb = [pos-20*vx ; pos+20*vx] ;
	elseif op == 2, stMLI.bb = [pos-10*vx ; pos+10*vx] ;
	elseif op == 1; stMLI.bb = [pos- 5*vx ; pos+ 5*vx] ;
