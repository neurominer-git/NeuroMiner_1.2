function varargout = nk_orthviews(action,varargin)
...
if ~isfield(st,'vols'),reset_st; end
spm('Pointer','watch');
...
if ~isempty(h),
	...
	spm_XYZreg('Add2Reg',st.registry.hReg,st.registry.hMe, 'nk_orthviews');
else
...
V.ax = cell(3,1);
for i=1:3,
    if isfield(st,'NMaxes')
        ax = st.NMaxes(i);
        set(ax,'Visible','off','SortMethod','childorder','Parent',st.fig,'DeleteFcn',DeleteFcn, ...
        ...
        ax = axes('Visible','off','SortMethod','childorder','Parent',st.fig,'DeleteFcn',DeleteFcn,...
            'YDir','normal','ButtonDownFcn',...
            ['if strcmp(get(gcf,''SelectionType''),''normal''),nk_orthviews(''Reposition'');',...
            'elseif strcmp(get(gcf,''SelectionType''),''extend''),nk_orthviews(''Reposition'');',...
            'nk_orthviews(''context_menu'',''ts'',1);end;']);
...
global st
if isfield(st,'ParentFig')
    obj    = get(st.ParentFig,'CurrentObject');
else
...
if isfield(st,'NMaxes')
    st      = struct('n', 0, 'vols',[], 'bb',bb,'Space',eye(4),'centre',[0 0 0],'callback',';','xhairs',1,'hld',1,'fig',st.fig,'mode',1,'plugins',[],'snap',[], 'NMaxes', st.NMaxes, 'ParentFig', st.ParentFig);
else
    if ~isfield(st,'fig'), st.fig = spm_figure('Findwin','Graphics'); end
    st      = struct('n', 0, 'vols',[], 'bb',bb,'Space',eye(4),'centre',[0 0 0],'callback',';','xhairs',1,'hld',1,'fig',st.fig,'mode',1,'plugins',[],'snap',[]);
end
st.vols = cell(24,1);
