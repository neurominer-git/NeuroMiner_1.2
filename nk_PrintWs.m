...
if nvar > 1
    if oTrainParam.STACKING.flag == 1
        fprintf('WORKSPACE VIEWER: ');
        fprintf('STACKING OPERATES ON ANALYSES %s ', strjoin(string(oTrainParam.STACKING.sel_anal),', ')); 
    else
        ...
        fprintf('WORKSPACE VIEWER: %s ', strmod); 
        for j=1:nF
            if j > showmodalmax, fprintf(' ...'); break; end
            fprintf('#%g ', oTrainParam.FUSION.M(j)); 
        end
    end
    fprintf('\n') 
end
