...
if ~exist('meanfun','var'), meanfun=@nm_nanmedian; end
if isempty(EVALFUNC)
     switch MODEFL
     ...
     [~,~,EVALFUNC] = nk_GetScaleYAxisLabel(GridParam); 
...
for curlabel=1:nl
    ...
    switch MODEFL
        ...
        case 'regression'
            ...
            if ~isempty(label)
                results.costfun_crit(curlabel) = EVALFUNC(xlabl, pred(ind));
                if numel(pred(ind))>1,
        ...
        case 'classification'
            ...
            if ~isempty(label)
                if ~isempty(optcutoff),
                    ...
                    results.costfun_crit = EVALFUNC(xlabl, pred(ind));
                    ...
                    results.costfun_crit = EVALFUNC(xlabl, pred(ind));
                    results.contigency = ALLPARAM( xlabl, pred(ind)); 
                    results.prob_contigency = ALLPARAM( xlabl, prob(ind,1)-0.5);  
                end
            end
