...
global VERBOSE TRAINFUNC EVALFUNC
...
switch r.WeightSort 
    ...
    case 1 %% Sorting is done according to CV1 test performance
        ...
        xk = 1; 
        valSp = zeros(nSp+1,1);
        iUSp = valSp; iLSp= valSp;
        S = r.StartExam; kS = S;
        tY = r.Y(:,kS);T = r.T(:,kS);
        [~, model] = TRAINFUNC(tY, label, 1, Ps);
        [valSp(1), ~, D] = nk_GetTestPerf(T, r.L, [], model, tY);
        iUSp(1) = max(D); iLSp(1) = min(D);
        
        while xk <= nSp

            Dxk = D; 
            iUSp(xk+1) = percentile(Dxk,r.CaseSpU(xk));
            iLSp(xk+1) = percentile(Dxk,r.CaseSpL(xk));
            Ixk = Dxk <= iUSp(xk+1) & Dxk >= iLSp(xk+1);
            ...
            while k > r.MinNum
                ...
                while lc > 0
                    ...
                    tY = r.Y(:,kS); T = r.T(:,kS);
                    [~, model] = TRAINFUNC(tY, label, 1, Ps);
                    [~, ~, Dxki(Ixk)] = nk_GetTestPerf(T(Ixk,:), r.L(Ixk), [], model, tY);
                    val(lc) = EVALFUNC(r.L, Dxki); 
                    lc = lc - 1;
                    
                end

                 % Sort features and thresholds
