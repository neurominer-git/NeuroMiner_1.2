function [ bbq ] = buildBBQ( PTR, YTR, options )
% Input:
...
% Options: Structure of optional choices
    ...
    opt1.N0 = N0;
    switch scoringFunc
        case 'BDeu' % N0 = 2 by default
            scoreFunc = @getBDeuScore_local;
        case 'BDeu2' % N0 = 2B by default (the version that used in the paper)
...
function [ histModel, cutIdx, cutPoints, logLikelihood ] = HistCalibrationFreq_local( PTR, YTR , b)
    ...
    if b==1
        ...
        histModel{1}.min = 0;
        if ~isfield(histModel{1},'max')
            if b>1
                histModel{1}.max = (T{1}.Yhat(end)+T{2}.Yhat(1))/2;
            else
                histModel{1}.max = T{1}.Yhat(end);
            end
        end
