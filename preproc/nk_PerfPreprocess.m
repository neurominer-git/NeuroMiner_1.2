    ...
    for l=sta_jy:stp_jy % Inner CV fold loop
        ...
        for u=1:ukbin % Binary comparison loop depending on PREPROC.BINMOD & FBINMOD
            ...
            if iscell(usY)
                ...
                % Independent test data
                if ~isempty(Yocv), [vTs{4},~, iOCV] = nk_ManageNanCases(usYocv); end
                % Calibration data
            ...
            if ~isempty(Yocv), ocv = squeeze(InputParam.Ts(:,4,:)); end
            ...
            switch BINMOD
                ...
                case 1   
                    ...
                    [tY.CV{k,l}{u},CVL] = nk_ManageNanCases(cvd, CVL, SrcParam.iCV); 
                    tY.Ts{k,l}{u} = nk_ManageNanCases(tsd, [], SrcParam.iTs);
                    if ~isempty(Yocv), tYocv.Ts{k,l}{u} = nk_ManageNanCases(ocv, [], SrcParam.iOCV); end
