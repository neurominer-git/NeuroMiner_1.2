...
for i=1:nP % Loop through hyperparameter combinations
    ...
    % Do the magic
    if fromData
        [R(i), R95CI(:,i)] = compute_perf(P(i,1), P(i,2), P(i,3), P(i,4), P(i,5), P(i,6), P(i,7), P(i,8), P(i,9), P(1,10), IN.algorithm, IN.RAND, IN.verbose, IN.Data, IN.DataLabel, IN.NRanalysis);
    else
...
if fromData
    ...
    for k=1:reps
        ...
        path = '/Users/claravetter/Documents/Documents_Clara_MacBookAir/LMU/Simulations/origData.csv';
        if isa(varargin{1,2},'double')
            varargin{1,2} = array2table(varargin{1,2});
        end
        writetable(varargin{1,2}, path);
        M_file = pyrunfile('py_simulate_data.py', 'out_path', ...
            data_file = path, ...
            labels = int64(num_labels), ...
            n_obs = int64(nc));
        %M = readtable(simData_file);
